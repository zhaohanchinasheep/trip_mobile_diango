...
class Slider(CommonModel):
    ...
    types = models.SmallIntegerField('展现的位置', default=10, help_text='10表示首页')
...
class ImageRelated(CommonModel):
    """图片关联表"""
    img = models.ImageField('图片地址', upload_to='%Y%m/file/', max_length=256)
    # 与用户表做外键关联
    user = models.ForeignKey(User, on_delete=models.SET(None),related_name="upload_images")
    summary = models.CharField('图片说明', max_length=64, null=True, blank=True)
    # 与数据库中的content_type表做连接
    content_type = models.ForeignKey(ContentType,on_delete=models.CASCADE)
    object_id = models.IntegerField('关联的模型id')
    # 从object_id找到对应的表
